...
static inline void outc(cmark_renderer *renderer, cmark_node *node,
  ...
  if (needs_escaping) {
    if (escape == URL && cmark_isspace((char)c)) {
      ...
      char nbuf[16];
      int nbuf_len = cmark_itoa(nbuf, c);
      cmark_strbuf_puts_lit(renderer->buffer, "&#");
      cmark_strbuf_put(renderer->buffer, (const unsigned char *)nbuf, nbuf_len);
      cmark_strbuf_putc(renderer->buffer, ';');
      renderer->column += 2 + nbuf_len + 1; // "&#" + digits + ";"
    }
  } else {
    cmark_render_code_point(renderer, c);
  }
