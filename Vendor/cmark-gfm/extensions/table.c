...
static const struct { const char *style; int style_len; const char *align; int align_len; }
S_table_align[] = {
  {" style=\"text-align: left\"", 25, " align=\"left\"", 13},
  {" style=\"text-align: center\"", 27, " align=\"center\"", 15},
  {" style=\"text-align: right\"", 26, " align=\"right\"", 14},
};

static void html_table_add_align(cmark_strbuf* html, int align_idx, int options) {
  if (options & CMARK_OPT_TABLE_PREFER_STYLE_ATTRIBUTES) {
    cmark_strbuf_put(html, (const unsigned char *)S_table_align[align_idx].style,
                     S_table_align[align_idx].style_len);
  } else {
    cmark_strbuf_put(html, (const unsigned char *)S_table_align[align_idx].align,
                     S_table_align[align_idx].align_len);
  }
}
...
static void html_table_add_spans(cmark_strbuf *html, unsigned colspan, unsigned rowspan) {
  if (colspan > 1) {
    char n[16];
    int n_len = cmark_itoa(n, (int)colspan);
    cmark_strbuf_puts_lit(html, " colspan=\"");
    cmark_strbuf_put(html, (const unsigned char *)n, n_len);
    cmark_strbuf_putc(html, '"');
  }
  if (rowspan > 1) {
    char n[16];
    int n_len = cmark_itoa(n, (int)rowspan);
    cmark_strbuf_puts_lit(html, " rowspan=\"");
    cmark_strbuf_put(html, (const unsigned char *)n, n_len);
    cmark_strbuf_putc(html, '"');
  }
}
...
static void html_render(cmark_syntax_extension *extension,
  ...
  if (node->type == CMARK_NODE_TABLE) {
    if (entering) {
      ...
      cmark_strbuf_puts_lit(html, "<table");
      ...
      if (table_state->need_closing_table_body) {
        cmark_html_render_cr(html);
        cmark_strbuf_puts_lit(html, "</tbody>");
        cmark_html_render_cr(html);
      }
      table_state->need_closing_table_body = false;
      cmark_html_render_cr(html);
    ...
    if (entering) {
      ...
      if (((node_table_row *)node->as.opaque)->is_header) {
        ...
        cmark_strbuf_puts_lit(html, "<thead>");
        cmark_html_render_cr(html);
    ...
    if (colspan > 0 && rowspan > 0) {
      if (entering) {
        ...
        if (table_state->in_table_header) {
          cmark_strbuf_puts_lit(html, "<th");
        } else {
          cmark_strbuf_puts_lit(html, "<td");
        }

        switch (get_cell_alignment(node)) {
          case 'l': html_table_add_align(html, 0, options); break;
          case 'c': html_table_add_align(html, 1, options); break;
          case 'r': html_table_add_align(html, 2, options); break;
        }
