...
static void commonmark_render(cmark_syntax_extension *extension,
  ...
  if (entering) {
    ...
    cmark_strbuf_puts_lit(renderer->prefix, "  ");
  } else {
...
static void html_render(cmark_syntax_extension *extension,
  ...
  if (entering) {
    ...
    cmark_strbuf_puts_lit(renderer->html, "<li");
    cmark_html_render_sourcepos(node, renderer->html, options);
    cmark_strbuf_putc(renderer->html, '>');
    if (node->as.list.checked) {
      cmark_strbuf_puts_lit(renderer->html, "<input type=\"checkbox\" checked=\"\" disabled=\"\" /> ");
    } else {
      cmark_strbuf_puts_lit(renderer->html, "<input type=\"checkbox\" disabled=\"\" /> ");
    }
  } else {
    cmark_strbuf_puts_lit(renderer->html, "</li>\n");
  }
